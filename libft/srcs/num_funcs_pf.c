...
/*   Updated: 2020/01/29 13:28:59 by sikpenou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_itoa_base_s(long long int n, char *base_to, t_buf *buf, t_arg *arg)
{
	...
	keep = pos;
	//printf("pos: %d, buf pos: %ld, arg->max: %ld\n", pos, buf->pos, arg->max);
	//while (pos > 0 && buf->pos - pos > arg->max)
	while (pos > 0 && pos > arg->max + buf->pos)
	{
		if (buf->str[pos - 1] != '-')
