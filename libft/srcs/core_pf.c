...
/*   Updated: 2020/01/29 13:09:56 by sikpenou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		init_structs(t_buf **buf, t_arg **arg)
{
	if (!(*buf = (t_buf *)easymalloc(sizeof(**buf))))
		return (0);
	(*buf)->tab[PERTYPE] = &f_percent;
	(*buf)->tab[CTYPE] = &f_c;
	(*buf)->tab[STYPE] = &f_s;
	(*buf)->tab[PTYPE] = &f_p;
	(*buf)->tab[DTYPE] = &f_num_s;
	(*buf)->tab[UTYPE] = &f_num_u;
...
int		format_to_str(const char *str, t_buf *buf, t_arg *arg, va_list arg_lst)
{
	...
	while (str[str_pos])
	{
		...
		{
			set_arg(arg);
			if ((ret = parse_pf_arg(buf, arg, str + str_pos + 1, arg_lst))
				== -1)
				return (-1);
			str_pos += ret + 1;
		}
